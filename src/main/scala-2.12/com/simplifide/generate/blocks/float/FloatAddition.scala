...
class FloatAddition(override val name:String,
                    ...
                    override val negative:Boolean) extends AdditionSegment with ConditionParser with SegmentHolder {
  ...
  def states(index:Int, inW:Int, outW:Int) = {
    ...
    /*val first =
      if (index == 1) internalSig.man ::= (abs_out(abs_out.width-2-index,0) >> posValue)+(abs_out(0)&abs_out(1))
      else internalSig.man ::= (abs_out(abs_out.width-2-index,0) << shiftValue)
    */
    ...
    //val second = internalSig.man ::= (abs_out(abs_out.width-2-index,abs_out.width-1-index-outW)) +
    //  (~abs_out(abs_out.width-1-index-outW-1) & abs_out(abs_out.width-1-index-outW-1) & abs_out(abs_out.width-1-index-outW) )
