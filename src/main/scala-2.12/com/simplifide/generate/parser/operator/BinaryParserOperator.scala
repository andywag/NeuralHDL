...
trait BinaryParserOperator extends Expression {
  ...
  private def createTotal(realIn1:SimpleSegment, realIn2:SimpleSegment) = {
    this match {
      ...
      case x:BinaryParserOperator.SSl           => new BinaryOperator.SSl(realIn1,realIn2)
      case x:BinaryParserOperator.SSr           => new BinaryOperator.SSr(realIn1,realIn2)
...
object BinaryParserOperator {
  ...
  class SSl(in1:Expression,in2:Expression) extends Base(in1,in2)
  class SSr(in1:Expression,in2:Expression) extends Base(in1,in2)
