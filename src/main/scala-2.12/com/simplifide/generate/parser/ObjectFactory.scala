...
import com.simplifide.generate.blocks.basic.condition.{ConditionStatementBuilder, QuestionSegment}
...
object ObjectFactory {
  ...
  def Flop(clk:Clock,output:Expression,input:Expression):SimpleSegment =
    FlopFactory(clk,output.create,input.create(output.create))


  def Flop(clk:Clock,output:Expression,reset:Expression,input:Expression) = {
    val realOutput = output.create
    val res =  List(new SimpleFlopList.Segment(realOutput,Some(reset.create(output))))
    val en  =  List(new SimpleFlopList.Segment(realOutput,Some(input.create(output))))
  ...
  */
  ...
  /*
  def Mult(lhs:Expression,rhs:Expression)(implicit clk:ClockControl):SimpleSegment =
    Mult(lhs,rhs,FixedType.Simple,FixedType.Simple)(clk)
  // Multiplier
  def Mult(lhs:Expression,rhs:Expression,fixed:FixedType,internal:FixedType)(implicit clk:ClockControl):SimpleSegment = {
    (lhs,rhs) match {
  ...
  def MultTrunc(lhs:Expression,rhs:Expression,fixed:FixedType,internal:FixedType)     =
    new MultiplySegment.Truncate("",lhs,rhs,fixed,internal)
  def MultTruncClip(lhs:Expression,rhs:Expression,fixed:FixedType,internal:FixedType) =
    new MultiplySegment.TruncateClip("",lhs,rhs,fixed,internal)
  def MultRound(lhs:Expression,rhs:Expression,fixed:FixedType,internal:FixedType)     =
    new MultiplySegment.Round("",lhs,rhs,fixed,internal)
  def MultRoundClip(lhs:Expression,rhs:Expression,fixed:FixedType,internal:FixedType) =
    new MultiplySegment.RoundClip("",lhs,rhs,fixed,internal)
  */
  // Division
  def Div(lhs:Expression,rhs:Expression)                            = null//factory.Mult(lhs,rhs)
  def DivTrunc(lhs:Expression,rhs:Expression,fixed:FixedType)     = null//factory.MultTrunc(lhs,rhs,fixed)
  def DivTruncClip(lhs:Expression,rhs:Expression,fixed:FixedType) = null//factory.MultTruncClip(lhs,rhs,fixed)
  def DivRound(lhs:Expression,rhs:Expression,fixed:FixedType)     = null//factory.MultRound(lhs,rhs,fixed)
  def DivRoundClip(lhs:Expression,rhs:Expression,fixed:FixedType) = null//factory.MultRoundClip(lhs,rhs,fixed)
  // Rounding
  ...
  */
  def Constant(name:String = "",value:Double,fixed:FixedType = FixedType.Simple) = {
