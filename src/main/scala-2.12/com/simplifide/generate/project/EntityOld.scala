...
import com.simplifide.generate.parser.{EntityParser, ModuleParser}

 /*
class Entity(val name:String)(implicit val clk:ClockControl) extends EntityParser with ControlBase {
  ...
  def writeModule(fileLocation:String):SegmentReturn = EntityGenerator(entity).writeModule(fileLocation)
...
object Entity {
  ...
  abstract class Leaf(name:String)(override implicit val clk:ClockControl) extends Entity(name) {

  }

  class Branch(name:String)(override implicit val clk:ClockControl) extends Entity(name) {
