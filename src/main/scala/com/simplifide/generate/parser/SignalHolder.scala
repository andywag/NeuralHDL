...
trait SignalHolder extends SignalMethods{
  ...
  def seq[T <: SignalTrait](proto:T, opType:OpType=OpType.Signal)(length:Int):Seq[T] = {
    // Fixme : Kludge forcing the type : Need generic copy of signal traits
    val results = Seq.tabulate(length)(x => proto.newSignal(name = proto.appendName(s"w$x"),opType).asInstanceOf[T])
    signals.appendAll(results)
    results
  }
