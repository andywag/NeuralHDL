...
class RunVerilator(test:Test, testCase:TestEntityParser, project:Project) {
  ...
  val verilatorCommand = s"${verilatorRoot}/verilator_bin --trace --trace-structs -Wall -Wno-COMBDLY -Wno-fatal -Wno-lint -Idesign -cc test/${entity.name}.v --exe test/${entity.name}.cpp"
