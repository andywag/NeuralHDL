...
case class DataSet[T](data:Seq[DataTrait[T]], signal:SignalTrait, gen:DataTrait[T], location:Option[String] = None) {
  ...
  def matchSet(x:DataSet[T], delay:Int) = {
    def chop[S](x:Seq[S],ignore:Int) = x.slice(ignore,x.length)
    ...
    val results = chop((this.data zip x.data),delay).zipWithIndex.map(x => x._1._1.compare(x._1._2,x._2))
  ...
  def delay(index:Int) = {
    if (index > 0) {
      val newData = this.data.slice(0,index).toList ::: this.data.toList
